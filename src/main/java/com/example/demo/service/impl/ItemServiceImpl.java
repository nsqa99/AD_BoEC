...
public class ItemServiceImpl implements ItemService {
	...
	public ItemDto insert(ItemDto dto) {
		...
		if (imageUrls != null && imageUrls.size() > 0) {
			images = imageUrls.stream().map(image -> new Image(image)).collect(Collectors.toList());
		}
	...
	public ItemDto update(ItemDto dto) {
		...
		if (imageUrls != null && imageUrls.size() > 0) {
			...
			for (int i=0; i<imageUrls.size(); i++) {
				if (i<itemImages.size()) {
					itemImages.get(i).setUrl(imageUrls.get(i));
					images.add(itemImages.get(i));
					itemImages.remove(i);
				} else {
					images.add(new Image(imageUrls.get(i)));
				}
			}
			while (itemImages.size() > 0) {
				imageRepos.delete(itemImages.get(0));
				itemImages.remove(0);
			}
		}
		Set<String> authors = new HashSet<>();
		...
		if (imageUrls != null && imageUrls.size() > 0) {
			List<Image> newImages = images.stream().map(image -> {
				if (image.getItem() == null) image.setItem(newItem);
				return imageRepos.save(image);
			}).collect(Collectors.toList());
			newItem.setImages(newImages);
		}
